...
1. **Dirichlet 级数**：$F(s) = \sum_{n=1}^{\infty} \frac{a_n}{n^s}$，在 $\operatorname{Re}(s) > 1$ 上绝对收敛，$a_1 = 1$
...
> **广义黎曼猜想（对 Selberg 类）**：对于 Selberg 类中的每个函数 $F$，$F(s)$ 的所有非平凡零点满足 $\operatorname{Re}(s) = 1/2$。
