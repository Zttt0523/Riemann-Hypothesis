...
更精确的版本使用**对数积分** $\operatorname{Li}(x)$：

$$
\operatorname{Li}(x) = \int_2^x \frac{dt}{\ln t}
$$

$\operatorname{Li}(x)$ 是比 $x / \ln x$ 更好的近似。素数定理的等价陈述是：
...
**引理 1**：$\zeta(s)$ 在 $\operatorname{Re}(s) = 1$ 的直线上没有零点。
...
由于 $\zeta(1+it) \neq 0$ 是一个较弱的陈述，我们只能得到较弱的误差项：$\pi(x) = \operatorname{Li}(x) + O(x e^{-c\sqrt{\ln x}})$。

如果黎曼猜想成立，则可以获得极精确的误差估计：$\pi(x) = \operatorname{Li}(x) + O(\sqrt{x} \ln x)$。反之亦然——**黎曼猜想等价于素数定理的误差项达到 $O(x^{1/2+\varepsilon})$**。
...
> **本章要点**：素数定理 $\pi(x) \sim x/\ln x$ 由 Hadamard 和 de la Vallée Poussin 于 1896 年独立证明。证明的关键步骤是展示 $\zeta(s)$ 在 $\operatorname{Re}(s) = 1$ 上没有零点。Perron 公式和切比雪夫函数是证明的技术基础。PNT 给出了素数分布的"一阶近似"；黎曼猜想等价于该近似的误差项达到最低可能阶 $O(x^{1/2+\varepsilon})$。
