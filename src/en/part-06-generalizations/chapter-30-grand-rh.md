...
> **Generalized Riemann Hypothesis (GRH)**: For every $L$-function $F(s)$ in the Selberg class (Chapter 27), all non-trivial zeros of $F(s)$ satisfy $\operatorname{Re}(s) = 1/2$.
...
For each Dirichlet character $\chi$ modulo $q$, the non-trivial zeros of $L(s, \chi)$ satisfy $\operatorname{Re}(s) = 1/2$.

### Dedekind Zeta Functions

For any algebraic number field $K$, the Dedekind zeta function $\zeta_K(s)$ has all non-trivial zeros satisfying $\operatorname{Re}(s) = 1/2$. Since $\zeta_{\mathbb{Q}}(s) = \zeta(s)$, the Dedekind GRH directly includes the original RH.

### Automorphic $L$-Functions

For any automorphic representation $\pi$ (Chapter 27), the non-trivial zeros of $L(s, \pi)$ satisfy $\operatorname{Re}(s) = 1/2$.
...
- **Langlands' version of GRH**: not only requires $\operatorname{Re}(\rho) = 1/2$, but also demands that $L(s, \pi)$ satisfy precise functional equations across a broader class of test functions ("functoriality"). This stronger version is one of the central conjectures of the Langlands program.
...
- Extremely uniform distribution of primes in arithmetic progressions ($\operatorname{Re} = 1/2$ gives the optimal error)
...
Perhaps the ultimate proof will not be about $\zeta(s)$ itself at all, but about a broader principle — about symmetry, representation theory, and spectra — from whose unified perspective $\operatorname{Re}(s) = 1/2$ becomes an inescapable necessity, woven into the fabric of the $L$-function universe.

---

> **Key points**: GRH extends $\operatorname{Re}(s)=1/2$ to the entire $L$-function family: Dirichlet $L$-functions, Dedekind zeta functions, and automorphic $L$-functions. GRH implies extremely uniform prime distribution and optimal error in Chebotarev's density theorem. The Langlands program regards GRH as the critical test of the functoriality conjecture. The ultimate proof may come from a unified understanding of the entire $L$-function universe rather than from $\zeta(s)$ alone.
