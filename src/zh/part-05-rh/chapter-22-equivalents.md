...
$$
|\pi(x) - \mathrm{Li}(x)| < \frac{1}{8\pi} \sqrt{x} \ln x \quad \text{对所有 } x \geq 2657 \text{ 成立}
$$
...
具体地说，若存在 Hilbert 空间上的自伴算子 $H$，使得 $H$ 的特征值恰好为 $\zeta$ 的非平凡零点，则由于自伴算子的特征值均为实数，$\zeta$ 非平凡零点的虚部——通过某种变换后——将是实的，即 $\Re(\rho) = 1/2$。
