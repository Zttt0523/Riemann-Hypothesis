...
黎曼猜想的核心舞台是临界线 $\operatorname{Re}(s) = \frac{1}{2}$。沿着这条直线，$\zeta(\frac{1}{2} + it)$ 是 $t \in \mathbb{R}$ 的复值函数。
...
自黎曼以来，数字计算机已计算出天文数字级别的 $\zeta$ 零点——全部位于临界线上。截至 2020 年代，已验证超过 $10^{13}$ 个零点全部满足 $\operatorname{Re}(s) = 1/2$。仅有的离群零点将意味着计算机的计算有误，但**没有一个反例被发现**。
