...
$\xi$ 函数关于临界线 $\operatorname{Re}(s) = 1/2$ 对称。这是黎曼猜想自然性的直观来源：如果有零点偏离了临界线，其对称点也将是零点，由此产生的结构似乎"过于复杂"。
...
## 临界线 $\operatorname{Re}(s) = 1/2$

由函数方程知：若 $\rho$ 是 $\zeta(s)$ 的非平凡零点，则 $1-\rho$ 也是零点。因此关于临界线 $\operatorname{Re}(s) = \frac{1}{2}$，零点成对出现，除非 $\rho$ 恰好位于临界线上——此时 $\rho$ 与 $1-\rho$ 是复共轭关系（$\bar{\rho} = 1 - \rho$）。
...
函数方程的一个直接推论是：所有非平凡零点都位于**临界带** $0 < \operatorname{Re}(s) < 1$ 之内。证明思路是：当 $\operatorname{Re}(s) > 1$ 时，欧拉乘积绝对收敛，因此 $\zeta(s) \neq 0$。函数方程随之推出 $\zeta(s) \neq 0$ 对 $\operatorname{Re}(s) < 0$ 也成立（除了平凡零点）。因此非平凡零点只能存在于 $0 \leq \operatorname{Re}(s) \leq 1$ 之内。在 $\operatorname{Re}(s) = 0$ 和 $\operatorname{Re}(s) = 1$ 直线上无零点的事实分别由函数方程和 PNT 的引理 1 给出。
...
> **本章要点**：函数方程 $\xi(s) = \xi(1-s)$ 显示了 $\zeta(s)$ 关于临界线 $\operatorname{Re}(s) = 1/2$ 的对称性。$\xi$ 函数是整函数，去除了 $\zeta(s)$ 的极点和 $\Gamma$ 函数带来的平凡零点。平凡零点位于 $s = -2, -4, -6, \ldots$。所有非平凡零点位于临界带 $0 < \operatorname{Re}(s) < 1$ 内。函数方程是理解黎曼猜想自然性的核心——它暗示临界线是零点分布的"天然位置"。
