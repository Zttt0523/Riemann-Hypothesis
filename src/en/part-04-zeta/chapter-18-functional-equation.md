...
## The Critical Line $\operatorname{Re}(s) = 1/2$

From the functional equation: if $\rho$ is a non-trivial zero of $\zeta(s)$, then $1 - \rho$ is also a zero. Zeros therefore come in symmetric pairs about the critical line $\operatorname{Re}(s) = 1/2$, unless $\rho$ lies exactly on it — in which case $\rho$ and $1-\rho$ are complex conjugates ($\bar{\rho} = 1 - \rho$).
...
## The Critical Strip $0 < \operatorname{Re}(s) < 1$

A direct consequence of the functional equation: all non-trivial zeros lie within the **critical strip** $0 < \operatorname{Re}(s) < 1$. The reasoning: for $\operatorname{Re}(s) > 1$, the Euler product converges absolutely, so $\zeta(s) \neq 0$. The functional equation then forces $\zeta(s) \neq 0$ for $\operatorname{Re}(s) < 0$ (except for the trivial zeros). Non-trivial zeros are therefore forced into $0 \leq \operatorname{Re}(s) \leq 1$. The absence of zeros on $\operatorname{Re}(s) = 0$ and $\operatorname{Re}(s) = 1$ follows, respectively, from the functional equation and from Lemma 1 of the PNT proof (Chapter 15).
